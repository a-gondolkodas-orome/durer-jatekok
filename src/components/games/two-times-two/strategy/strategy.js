...
import { random, sum, isEqual, cloneDeep } from 'lodash-es';

export const generateNewBoard = () => ([0, 0, 0, 0]);

export const makeAiMove = (board) => {
  const tileIndex = getOptimalTileIndex(board);
  return getBoardAfterPlacingOnTile(board, tileIndex);
};

export const getBoardAfterPlacingOnTile = (board, tileIndex) => {
