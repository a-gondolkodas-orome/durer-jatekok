...
// Harder (category D) version of "10 coins": coin values run 1..5 instead of
// 1..4. The board is the multiset of coin values; only which distinct values are
// present matters for the game logic, the counts are pure flavour.
type Board = number[]

const totalCoins = 10;
const values = [1, 2, 3, 4, 5];

// --- Exact solver over the (at most 31) non-empty subsets of {1,2,3,4,5} -------
...
// With values 1..5 the losing positions (second player to move wins) are
// {1,2,3}, {1,4,5}, {2,3,4,5} and {1,2,3,4,5}. Everything else is a first-player
// win, driven towards one of these (or merged to a single value when two remain).
...
// Mix of starts: ~half are losing sets (second player wins), ~half are a
// first-player win, so choosing the right role genuinely matters.
const losingSets = [[1, 2, 3], [1, 4, 5], [2, 3, 4, 5], [1, 2, 3, 4, 5]];
const winningSets = [
  [1, 2, 4],
  [1, 3, 4],
  [2, 3, 4],
...
const generateStartBoard = (): Board => {
  const set = random(0, 1) === 0 ? sample(losingSets)! : sample(winningSets)!;
...
const getPlayerStepDescription = ({ ctx }) => {
  if (ctx.turnState !== null) {
    return {
      hu: `Válasszátok ki, mi legyen a(z) ${ctx.turnState} értékű érmék új (kisebb) értéke.`,
      en: `Choose the new (smaller) value for the coins worth ${ctx.turnState}.`
    };
  }
  return {
    hu: 'Válasszátok ki, melyik értékű érméket változtatjátok meg (egyszerre az összeset).',
...
const rule = {
  hu: <>
    ...
    Ti dönthetitek el, hogy a kezdő vagy a második játékos bőrébe szeretnétek-e bújni.
  </>,
  en: <>
    There are 10 coins on the table to start, each with an integer value between 1 and 5.
...
export const TenCoinsD = strategyGameFactory({
  presentation: {
    rule,
