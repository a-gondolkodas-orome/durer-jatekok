...
const hasFirstPlayerWon = (board) =>{
    if (( board.red === board.blue1 || board.red === board.blue2 ) && board.turnCount < 4 ){
        return true;
    }
    return false;
}
...
const makeOptimalStepAsFirst = (board) => {
  ...
  for (let i = 0; i < 3; i++){
    if (neighbours[board.blue2][i] === board.red && index1 !== neighbours[board.blue2][i]){
      ...
      for (let j = 0; j < 3; j++){
        if(neighbours[neighbours[board.blue2][i]][j] === board.red && index1 !== neighbours[board.blue2][i]){
          index2 = neighbours[board.blue2][i];
        }
      }
    }
    ...
    return getGameStateAfterMove(newBoard).newBoard;
};
