...
import {
  ...
  moves
} from "../bacteria/helpers";
import type { Board } from "./danger";

const boardWidth = 17;
const rowCount = 9;

// Curated starting positions: bacteria sit on the bottom row, goals (not
// necessarily adjacent) on the top row. The mix keeps each side winning
// roughly half the time. Verified in bot-strategy.spec.ts.
const startConfigs: { starts: number[], goals: number[] }[] = [
  // attacker-winning
...
const generateStartBoard = (): Board => {
  const bacteria = range(rowCount).map(
    rowIndex => Array(rowIndex % 2 === 0 ? boardWidth : boardWidth - 1).fill(0)
  );
  const { starts, goals } = sample(startConfigs)!;
...
const rule = {
  hu: <>
    ...
    a tábla felső sorában a megjelölt (nem feltétlenül szomszédos) mezők a CÉL mezők.
  ...
  en: <>
    ...
    in the top row — not necessarily adjacent — are GOAL squares.
...
export const BacteriaScatteredGoals = strategyGameFactory({
  presentation: {
    rule,
    roleLabels: [
