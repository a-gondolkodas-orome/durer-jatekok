...
const getOptimalThirdStep = (board) => {
  ...
  equivalentPositions.push([mb1, mb2]);
  equivalentPositions.push([mb2, mb1]);
  ...
  const pos = equivalentPositions.find(([[r1, c1], [r2, c2]]) =>
    aiOptimalThirdSteps[`${r1};${c1} - ${r2};${c2}`] !== undefined
  )
  if (pos !== undefined) {
    return aiOptimalThirdSteps[`${pos[0][0]};${pos[0][1]} - ${pos[1][0]};${pos[1][1]}`]
  } {
    return undefined;
  }
}
